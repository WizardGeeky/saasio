...
export const GET = withAuth(async (
    ...
    const deny = await checkPrivilege(_user, "GET", "/api/v1/private/cv-history");
    if (deny) return deny;
    ...
    try {
        ...
        // ── Aggregate global stats (CV-only: source="my-cvs-ai") ─────────────
        const CV_FILTER = { source: "my-cvs-ai" } as const;
        ...
        const [thisWeekCount, uniqueUsersArr, topTemplateArr] = await Promise.all([
            ResumeDownload.countDocuments({ ...CV_FILTER, createdAt: { $gte: thisWeekStart } }),
            ResumeDownload.distinct("userId", CV_FILTER),
        ...
        const aiGenerations         = agg?.aiGenerations         ?? 0;
        ...
        // ── Build per-request filter (always scoped to my-cvs-ai) ────────────
        const conditions: Record<string, any>[] = [{ source: "my-cvs-ai" }];
