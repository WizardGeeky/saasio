...
function SubscriptionBadge({ record }: { record: ResumeHistoryRecord }) {
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        if (!res.ok) {
          toastError(data.message ?? "Failed to load resumes history");
          return;
        }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-sm text-gray-500 mt-1">
          You do not have permission to view all resumes history.
        </p>
      </div>
    );
  }
